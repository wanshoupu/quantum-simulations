...
def test_convert_expansion():
    for _ in range(1):
        ...
        ctrlM = ControlledM.convert(unitM)
...
def test_init_qspace_seq():
    ...
    cu = ControlledM(m, controls, qspace)
...
def test_init_qspace_numpy_array():
    ...
    cu = ControlledM(random_unitary(2), controls, qspace=qids)
...
def test_sorted_4x4():
    ...
    qids = [3, 0]
    cu = ControlledM(m, controls, qids)
    # print()
    # print(formatter.tostr(cu.inflate()))
...
def test_sorted_noop():
    ...
    controls = (QType.CONTROL1, QType.CONTROL1, QType.TARGET)
    qids = list(range(3))
    expected = ControlledM(m, controls, qids)
    # print()
...
def test_matmul_identical_controls():
    ...
    t = random.randint(1, k)
    controls = random_control(k, t)
    a = ControlledM(random_unitary(1 << t), controls)
    b = ControlledM(random_unitary(1 << t), controls)
    c = a @ b
...
def test_matmul_identical_qspace_diff_controls():
    k = random.randint(2, 5)
    t = random.randint(1, k)
    a = ControlledM(random_unitary(2), random_control(k, t))
    b = ControlledM(random_unitary(2), random_control(k, t))
...
def test_matmul_verify_qspace():
    ...
    qid1 = [3, 0]
    a = ControlledM(random_unitary(4), controls, qspace=qid1)
    qid2 = [1, 0]
    b = ControlledM(random_unitary(4), controls, qspace=qid2)
...
def test_expand_eqiv_left_kron():
    ...
    a = ControlledM(unitary1, controls, qspace=qid1)
    print('a')
...
def test_matmul_uncontrolled_diff_qspace():
    controls = [QType.TARGET, QType.TARGET]  # all targets, no control
    a = ControlledM(random_unitary(4), controls, qspace=[1, 3])
    # print('a')
    # print(formatter.tostr(a.inflate()))

    b = ControlledM(random_unitary(4), controls, qspace=[3, 0])
    # print('b')
...
def test_matmul_random():
    for _ in range(10):
        ...
        a = ControlledM(random_unitary(1 << t), random_control(k, t), qspace=rqids())
        b = ControlledM(random_unitary(1 << t), random_control(k, t), qspace=rqids())
